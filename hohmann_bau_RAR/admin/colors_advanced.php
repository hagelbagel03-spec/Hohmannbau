        ...
        if (isset($_POST['action'])) {
            switch ($_POST['action']) {
                ...
                case 'update_colors':
                    ...
                    $service_description_color = sanitizeInput($_POST['service_description_color'] ?? '#374151');
                    ...
                    if ($stmt->execute([$footer_bg, $footer_text, $header_bg, $header_text, 
                                      ...
                                      $heading_color, $subheading_color, $link_color, $highlight_color, $service_description_color])) {
...
try {
    ...
    $service_description_color = $homepage['service_description_color'] ?? '#374151';
} catch (Exception $e) {
            ...
            <div class="admin-card p-6">
                ...
                <form method="POST">
                    ...
                    <div class="space-y-4">
                        ...
                        <!-- SPEZIELLE TEXT-FARBEN SEKTION - NEU -->
                        <div class="border border-green-200 rounded-lg p-4 bg-green-50">
                            <h3 class="font-semibold mb-3 flex items-center">
                                <i class="fas fa-quote-left mr-2 text-green-600"></i>
                                🌿 Spezifische Texte - "Von der Planung bis zur Pflege"
                            </h3>
                            <div class="space-y-3">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Service-Beschreibung Textfarbe</label>
                                    <input type="color" name="service_description_color" value="<?php echo $subheading_color; ?>" 
                                           class="w-full h-12 border border-gray-300 rounded cursor-pointer">
                                    <input type="text" name="service_description_color_display" value="<?php echo $subheading_color; ?>" 
                                           class="w-full mt-1 px-2 py-1 border border-gray-300 rounded text-sm" readonly>
                                </div>
                                <div class="bg-white p-3 rounded border">
                                    <p class="text-sm font-medium text-gray-600 mb-2">Vorschau:</p>
                                    <p id="service-preview" style="color: <?php echo $subheading_color; ?>; font-size: 18px; font-weight: 500;">
...
document.querySelectorAll('input[type="color"]').forEach(function(input) {
    input.addEventListener('change', function() {
        ...
        // Live preview for service description color
        if (this.name === 'service_description_color') {
